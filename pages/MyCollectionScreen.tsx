		...
		try {
			...
			const collectionsWithCount = await Promise.all(
				(collectionsData || []).map(async (collection) => {
					...
					if (countError) {
						console.error(
							"Error counting recipes for collection:",
							collection.id,
							countError
						);
					}
			...
			const filtered = collections.filter(
				(collection) =>
					...
					(collection.description &&
						collection.description.toLowerCase().includes(text.toLowerCase()))
			);
		...
		<View className='flex-1 items-center justify-center px-6 py-12'>
			<View className='items-center'>
				...
				<Text className='text-gray-500 text-center mb-6 leading-5'>
					Create your first collection to organize your recipes by categories,
					occasions, or any way you like!
				</Text>
				<TouchableOpacity
			...
			>
				<View className='px-4 py-4'>
					<SearchBarTW
						...
						containerClassName='w-full border-hairline border-blue-500'
					/>
				</View>
