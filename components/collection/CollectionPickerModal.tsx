	...
	const addToCollection = async (collectionId: string) => {
		...
		try {
			...
			if (existing) {
				Alert.alert(
					"Info",
					"This recipe is already in the selected collection"
				);
				return;
			} // Add recipe to collection with current timestamp for created_at
				...
				<View className='bg-white border-b border-gray-200 pt-12 pb-4 px-4'>
					...
					{recipe && (
						<View className='mt-3 p-3 bg-blue-50 rounded-lg'>
							<Text className='font-medium text-blue-800'>{recipe.title}</Text>
							{recipe.description && (
								<Text
